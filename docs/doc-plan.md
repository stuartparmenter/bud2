...
| 2 | Zettel Library Discovery & Generation | 0.48 | `internal/executive/simple_session.go`, `cmd/bud/main.go` | no doc, generated at startup from plugin manifests; cache-readonly semantics; 117 commits/90d. (foundational) | missing |
...
Run `dev:arch-doc "Zettel Library Discovery & Generation"` on `bud2` — `generateZettelLibraries` is new (added in the last 7 commits), architecturally significant (startup side-effect, readonly semantics for cache clones), and has no dedicated doc.
