...
| `internal/executive/executive_v2.go` | Core orchestrator — runs Claude sessions, manages signal_done, subagents, per-prompt agent/skill reload, memory quality feedback |
...
| `seed/` | Core bundled plugins (`bud`, `bud-ops`), guides, reflexes, wakeup/startup instructions |
...
- `seed/startup-instructions.md` — injected into daemon startup prompts; instructs Claude to check for interrupted subagents and review handoff notes
...
- **Changing how Claude is prompted**: `internal/executive/executive_v2.go` around `buildPrompt` — context assembly is here; also check `seed/core.md`, `seed/wakeup.md` (autonomous wakes), and `seed/startup-instructions.md` (daemon startup)
