...
- `internal/executive/agent_defs.go` — plugin-aware agent definition loading; applies tool grants from extensions.yaml
...
- `state/system/extensions.yaml` — extensions manifest: `plugins:` declares external plugins (GitHub repos, local paths) with tool grants and exclude lists; `skills:` declares standalone skills from ClaWHub (`clawhub:slug[@ver]`), GitHub (`git:owner/repo`), or local paths
...
- **Patterns to know**: Core defaults (`bud`, `bud-ops` plugins, guides, reflexes, wakeup/startup instructions) live in `state-defaults/system/` — these are the shipping defaults. The `paths` package implements a read-time overlay: `state/system/<path>` overrides `state-defaults/system/<path>` when present; if not, the default is served directly without copying. This replaced the old seed-copy-on-startup model (commits 49aa775, 6efc99f). Config is loaded from `bud.yaml` (gitignored, specified via `--config` or `BUD_CONFIG`); `bud.yaml.example` is the reference. Terminal manager (zellij or tmux) is set via `terminal_manager:` in `bud.yaml`, defaulting to zellij. Extensions are declared in `state/system/extensions.yaml` (replaces `plugins.yaml`; the old name still works with a deprecation warning). The `plugins:` section clones/updates GitHub repos at startup (auto-readonly) or loads local paths; use `git:owner/repo` or `path:/local/path` prefixes. The `skills:` section loads standalone skills from ClaWHub (`clawhub:slug[@ver]`), GitHub repos, or local paths — ClaWHub skills are downloaded as a zip and cached locally; floating versions re-fetch based on `extensions.update_interval` in `bud.yaml` (default: 1h). The `exclude:` list on a `plugins:` entry skips named sub-plugin directories. At startup, `generateZettelLibraries` scans all plugin manifests for `"zettels"` declarations and writes `state/system/zettel-libraries.yaml`; manual entries (no `source:` field) are preserved across restarts. Agent definitions and skills hot-reload from plugins on every prompt without restart. The `state/` directory is Bud's working directory (`~/Documents/bud-state`) and is a separate git repo. Memory retrieval is disabled for startup impulses; the limit is 6 engrams for regular sessions. After `signal_done`, recalled memory quality ratings are sent back to Engram via `RateEngrams()`. Profiling is disabled by default — enable via `BUD_PROFILE=minimal|detailed|trace` (currently commented out, re-enable by uncommenting `main.go` block). OS-aware `STATE_PATH` defaults: `~/Documents/bud-state` (macOS), `$XDG_DATA_HOME/bud/state` or `~/.local/share/bud/state` (Linux).
...
- **Adding/configuring a plugin**: `state/system/extensions.yaml` `plugins:` section — add `git:owner/repo` or `path:/local/path`; use `tool_grants` to control which MCP tools the plugin's agents can call, `exclude:` to skip specific sub-plugin directories
- **Adding a standalone skill**: `state/system/extensions.yaml` `skills:` section — `clawhub:slug[@version]` for ClaWHub skills (browse at https://clawhub.ai), `git:owner/repo[:dir][@ref]` for GitHub, `path:/local/path` for local; configure re-fetch interval via `extensions.update_interval` in `bud.yaml`
